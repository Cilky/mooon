...
/**
 * A class representing a two-dimensional vector of {@code float}s. Useful when working with
 * positions and sizes in a continuous space, such as a game world.
 * 
 * {@code Vec2f} instances, like {@link String}s, are immutable, and should be treated in a similar
...
public final class Vec2f implements Serializable {
	...
	/**
	 * Since {@link Vec2f} instances are immutable, their x and y fields may be accessed without
	 * getters.
	 */
	...
	/**
	 * Constructor. Creates a new instance by converting the components of a {@link Vec2i} to
	 * {@code float}s.
	 * @param i      the {@code Vec2i} from which to copy the values
	 */
	 ...
	 * @param radians      the angle, in radians, of the vector from the x axis
	 * @param magnitude    the magnitude of the vector
	 * @return             a {@code Vec2f} that forms a {@code radians}-radian angle counter-
	 *                     clockwise from the x axis with a magnitude of {@code magnitude} 
	 */
	public final static Vec2f fromPolar(float radians, float magnitude) {
		return new Vec2f((float)(magnitude*cos(radians)), (float)(magnitude*sin(radians)));
	}
	 ...
	 * @return       a new {@link Vec2f} instance where each component has been multiplied by
	 *               {@code s}
	 */
	 ...
	 * @return       a new {@link Vec2f} instance where each component has been multiplied by the
	 ...
	 * @return		a new {@link Vec2f} instance where each component has been divided by
	 * 				{@code s}
	 */
	 ...
	 * @return       a new {@link Vec2f} instance where each component has been divided by the
	 *               corresponding component in {@code v}
	 */
	 ...
	 * @return       a new {@link Vec2f} instance where each component has added the corresponding
	 *               component in {@code v}
	 */
	 ...
	 * @return       a new {@link Vec2f} instance where each component has subtracted the corresponding
	 *               component in {@code v}
	 */
	...
	/**
	 * Takes the {@linkplain Math#floor(double) floor} of the components of this vector.
	 * @return      a new {@link Vec2f} instance where each component is the floor of the components
	 *              of this vector
	 */
	...
	/**
	 * Takes the {@linkplain Math#ceil(double) ceiling} of the components of this vector.
	 * @return      a new {@link Vec2f} instance where each component is the ceiling of the components
	 *              of this vector
	 */
	...
	/**
	 * Linearly interpolates between two vectors. For example, a {@code frac} value of 0 would
	 * return a vector with the same value as this vector, whereas a value of 1 would return a
	 * vector with the same value as {@code dest} and a value of .4 would return a vector four
	 * tenths of the way from this vector to {@code dest}.
	 * @param dest      the vector to which to linearly interpolate from this vector
	 * @param frac      the fraction of the distance from this vector to {@code dest} to go
	 * @return          a vector that is {@code frac} amount of the distance between this vector
	 *                  and {@code dest}  
	 */
	...
	/**
	 * Returns a normalized (same direction but with a magnitude of 1) version of this vector.
	 ...
	 * @param p1      a point on the line onto which to project 
	 * @param p2      another point on the line onto which to project
	...
	public final Vec2f projectOntoLine(Vec2f p1, Vec2f p2) {
		...
		return p1.plus(between.smult(this.minus(p1).dot(between) / between.mag2()));
	}
	
	/**
	 * Returns the component of this vector along the axis specified by {@code other}.
	 * @param other   the axis onto which to project this vector
	 ...
	 * @return       the angle this vector forms with the x axis, in the range [0,2*PI)
	 */
	...
	/**
	 * Returns the dot product of this vector and {@code v}. If you are not sure what a dot product
	 * is, please speak to a TA.
	 * @param v      the vector with which to take the dot product
	...
	/**
	 * Returns the SQUARED magnitude (also called the length) of this vector. Useful for a performance
	 * increase whenever magnitudes are only being compared, relatively, rather than when the actual
	 * magnitude is needed. 
	...
	/**
	 * Returns the magnitude (also called the length) of this vector. If the magnitude will only be
	 * used for comparison (i.e. if the actual magnitude is not needed), consider using
	 * {@link #mag2() mag2} instead.
	...
	/**
	 * Returns the SQUARED distance between this vector and {@code v}. Useful for a performance
	 * increase whenever distances are only being compared, relatively, rather than when the actual
	 * distance is needed. 
	 * @param v     the vector to get the squared distance between
	...
	/**
	 * Returns the distance between this vector and {@code v}. If this distance will only be used
	 * for comparison (i.e. if the actual distance is not needed), consider using
	 * {@link #dist2(Vec2f) dist2} instead.
	 * @param v     the vector to get the distance between
